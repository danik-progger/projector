...
func getArgs(opts *Opts) ([]string, error) {
	...
	if operation == Add {
		if len(opts.Args) != 3 {
			return nil, fmt.Errorf("add command requires 2 arguments, got %v", len(opts.Args)-1)
		}
		return opts.Args[1:], nil
	}
	if operation == Remove {
		if len(opts.Args) != 2 {
			return nil, fmt.Errorf("remove command requires 1 argument, got %v", len(opts.Args)-1)
		}
		return opts.Args[1:], nil
	}

	if len(opts.Args) > 1 {
		return nil, fmt.Errorf("print command requires 0 or 1 argument, got %v", len(opts.Args)-1)
	}
